...
if (slider && next && prev) {
    ...
    // Tự động trượt ảnh
    let autoSlideInterval = setInterval(scrollToNext, 3000);
...
function toggleFAQ(button) {
  const parentDiv = button.closest('div').parentElement; // div lớn chứa cả câu hỏi + trả lời
  const answer = parentDiv.querySelector('.faq-answer');
  if (answer) {
    answer.classList.toggle('hidden');
  }
}
// gửi mail
const scriptURL = 'https://script.google.com/macros/s/AKfycbzvFSb6xaKlOSDJwk5ecBq6eI_De1g-tW4RlPXlvN0LXChl00rEYPlxN5Hy-HVBW5GjOg/exec';
...
//   E_BOOK
const scriptURLebook = 'https://script.google.com/macros/s/AKfycbzxE1wbkbeuWzjvPEd0PQ-wIFCuYAfgSWnWUDe7McrqpDhAr30Qd-JFx_WFI7-fUBZQ/exec';
...
const swiper = new Swiper('.mySwiper', {
  ...
  effect: 'fade', // Hiệu ứng chuyển slide là fade
  fadeEffect: {
    crossFade: true,
  },
});

// Lắng nghe sự kiện slide change để cập nhật chấm
swiper.on('slideChange', function () {
  const dots = document.querySelectorAll('.dot');
  // Xóa lớp 'active' khỏi tất cả các chấm
  dots.forEach(dot => dot.classList.remove('active'));
  // Thêm lớp 'active' vào chấm tương ứng với slide hiện tại
